...
func prepareAction(commandName string, c *cli.Context) (context.Context, error) {
	...
	log.Printf("info: starting %s.%s-%s", revproxy.AppName, commandName, revproxy.Version)
...
func main() {
	...
	app := cli.NewApp()
	app.Version = revproxy.Version
	app.Name = revproxy.AppName
	app.Usage = ""
	...
	app.Flags = []cli.Flag{
		cli.StringFlag{
			...
			Value:  "/tmp",
			EnvVar: "REVPROXY_LOG_DIR",
		},
		cli.StringFlag{
			Name:   "data-dir",
			Usage:  "data directory",
			Value:  "./data",
			EnvVar: "REVPROXY_DATA_DIR",
		},
		cli.IntFlag{
			Name:   "procs, P",
			Usage:  "attach cpu(s)",
			Value:  runtime.NumCPU(),
			EnvVar: "REVPROXY_NUM_CPU",
		},
		cli.BoolFlag{
			Name:   "debug, d",
			Usage:  "debug mode",
			EnvVar: "REVPROXY_DEBUG",
		},
		cli.BoolFlag{
			Name:   "verbose, V",
			Usage:  "verbose. more message",
			EnvVar: "REVPROXY_VERBOSE",
		},
	}
